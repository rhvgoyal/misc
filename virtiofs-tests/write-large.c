...
/* Try to write a large file one 2MB page at a time. */
...
#define RANGE_SIZE      (2*1024*1024)
	...
	for (i = 0; i < nr_ranges; i++) {
		/* Write few bytes */
		ret = pwrite(fd, buf, 15, i * RANGE_SIZE);
		if (ret == -1) {
			fprintf(stderr, "Failed to write %d bytes at page index=%d:%s, errorno=%d\n", 15, strerror(errno), i, errno);
		} else {
			printf("wrote %d bytes at page index=%d buf=[%s]\n",
				ret, i, buf);
		}
	}
	close(fd);
